...
fun main() {
    ...
    val map = IntArray(d + 1) { it }
    val sc = Array(10001) { mutableListOf<Pair<Int, Int>>() }

    repeat(n) {
        val (from, to, dis) = readln().split(" ").map { it.toInt() }
        sc[to].add(Pair(from, dis))
    }
    ...
    for (i in 1..d) {
        if (sc[i].isEmpty()) {
            ...
            for ((from, distance) in sc[i]) {
                map[i] = minOf(map[i], minOf(map[i - 1] + 1, map[from] + distance))
            }
        }
    }
