...
fun main() {
    ...
    val length = k.length / n.length
    val arr = mutableListOf<Triple<Char, Int, String>>()

    val temp = n.toCharArray().sorted()

    for (i in n.indices) {
        arr.add(Triple(temp[i], i, k.substring(length * i, length * i + length)))
    }

    arr.sortBy { it.second }
    val ans = mutableListOf<Triple<Char, Int, String>>()
