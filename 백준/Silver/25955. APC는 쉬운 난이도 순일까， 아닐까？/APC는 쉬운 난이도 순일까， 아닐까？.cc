...
난이도는 브론즈 티어, 실버 티어, 골드 티어, 플래티넘 티어, 다이아몬드 티어 순으로 어려워지며, 동일 난이도가 없도록 엄밀한 구별을 위하여 각각의 티어에 1에서 1\,000 사이의 수를 매겨놓기로 하였다. 동일 티어에서는 수가 작을수록 어려운 것을 의미한다. 예를 들어서 브론즈 티어 1 단계의 문제는 브론즈 티어 1\,000 단계의 문제보다 어렵고, 실버 티어 1\,000 단계의 문제는 브론즈 티어 1 단계의 문제보다 어렵다. 이후 선우는 문제를 난이도의 오름차순으로 정렬해놓았다.
...
bool cmp(const string &x, const string &y) {
    static string tiers = "BSGPD";  
    char tx = x[0], ty = y[0];
    if (tx != ty)
        return tiers.find(tx) < tiers.find(ty);
    int nx = stoi(x.substr(1)), ny = stoi(y.substr(1));
    return nx > ny;
}
