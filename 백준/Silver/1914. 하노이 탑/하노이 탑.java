...
public class Main {
   ...
   public static void main(String[] args) throws IOException {
      ...
      sb.append(num.pow(n).subtract(BigInteger.ONE)).append('\n');
      
      if (n <= 20) hinoi(n, 1, 3, 2);

      System.out.print(sb);
   }

    static void hinoi(int n, int from, int to, int by) {
      if (n == 0) return;
      hinoi(n-1, from, by, to);
      sb.append(from + " " + to).append('\n');
      hinoi(n-1, by, to, from);
    }
}